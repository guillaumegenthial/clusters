...
from utils.features_utils import matrix_extraction
...
for i in range(min(mytree.GetEntries(), 10)):
    ...
    leadjets    = get_leadjets(mytree, min_energy=20, max_eta=10)
    ...
    for j in range(len(mytree.Topocluster_E)):
        ...
        matrices = matrix_extraction(topo_cells, config.layer_extractors, topo_eta, topo_phi)
        for i in range(23):
